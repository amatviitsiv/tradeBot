...
class PaperBroker:
    ...
    def create_market_order(self, symbol: str, side: str, qty: float, price: float) -> Optional[Dict]:
        """
        qty = base asset qty. price = unit price in quote (USDT).
        We apply PAPER_TAKER_FEE to notional.
        """
        side = side.upper()
        base, quote = split_symbol(symbol)
        if base is None or quote is None:
            errors_logger.error(f"[PAPER] bad symbol {symbol}")
            return None
        ...
        if side == "BUY":
            ...
            if self.get_balance(quote) + 1e-9 < total_cost:
                logger.warning(f"[PAPER] insufficient {quote} ({self.get_balance(quote)} < {total_cost})")
                return None
            self.balances[quote] -= total_cost
            self.balances[base] = self.balances.get(base, 0.0) + qty
            rec = {"side": "BUY", "symbol": symbol, "qty": qty, "price": price, "notional": notional, "fee": fee}
            self._record_trade(rec)
            logger.info(f"[PAPER] BUY {symbol} {qty:.8f} @ {price:.2f} cost={total_cost:.2f} fee={fee:.4f}")
            return rec

        elif side == "SELL":
            pos_qty = self.get_balance(base)
            if pos_qty + 1e-9 < qty:
                logger.warning(f"[PAPER] insufficient {base} to SELL ({pos_qty} < {qty})")
                return None
            proceeds = qty * price
            fee = proceeds * cfg.PAPER_TAKER_FEE
            net = proceeds - fee
            self.balances[base] -= qty
            self.balances[quote] = self.balances.get(quote, 0.0) + net
            rec = {"side": "SELL", "symbol": symbol, "qty": qty, "price": price, "proceeds": proceeds, "fee": fee, "net": net}
            self._record_trade(rec)
            logger.info(f"[PAPER] SELL {symbol} {qty:.8f} @ {price:.2f} net={net:.2f} fee={fee:.4f}")
            return rec
        else:
            raise ValueError("side must be BUY or SELL")

    def get_pnl(self, symbol: str, price: float) -> Optional[Dict]:
        ...
        if qty <= 0:
            return None
        # compute avg buy price accounting fee from history
        buys = [h for h in self.history if h.get("symbol") == symbol and h.get("side") == "BUY"]
        if not buys:
            return None
        total_qty = sum(b["qty"] for b in buys)
        weighted = sum(b["qty"] * (b["price"] + (b.get("fee", 0) / b["qty"])) for b in buys) / total_qty
        pnl_usdt = (price - weighted) * qty
        pnl_pct = (price / weighted - 1) * 100
...
class LiveBroker:
    ...
    def _round_qty(self, symbol: str, qty: float) -> float:
        ...
        step = info.get("stepSize") if info else None
        if step:
            return round_down(qty, step)
        return qty

    async def create_market_order(self, symbol: str, side: str, qty: float, price: Optional[float] = None) -> Optional[Dict]:
        ...
        if min_qty and qty < min_qty:
            ...
            if adj <= 0:
                raise ValueError(f"{symbol} qty {qty} < minQty {min_qty}")
            logger.warning(f"[LIVE] {symbol} qty {qty} < minQty {min_qty}, adjusted -> {adj}")
            qty = adj
        ...
        if min_notional and notional < min_notional:
            ...
            if adj <= 0:
                raise ValueError(f"{symbol} notional {notional:.2f} < minNotional {min_notional}")
            logger.warning(f"[LIVE] {symbol} notional {notional:.2f} < minNotional {min_notional}, adjusted qty -> {adj}")
            qty = adj
        ...
        if side.upper() == "SELL":
            ...
            if bal < qty:
                logger.error(f"[LIVE] Not enough {base} to SELL ({bal} < {qty})")
                return None
        ...
        for attempt in range(cfg.ORDER_RETRY):
            try:
                ...
                await self.update_balances()
                logger.info(f"[LIVE] {side.upper()} {symbol} qty={qty:.8f}")
                trades_logger.info({"side": side.upper(), "symbol": symbol, "qty": qty, "price": price})
                return res
            except Exception as e:
                last_exc = e
                errors_logger.warning(f"[LIVE] order attempt {attempt+1} failed: {e}")
