...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Считаем индикаторы для стратегии:
    - EMA_Fast / EMA_Slow
    - SMA_TREND (например SMA200)
    - ATR (волатильность)
    - ADX (сила тренда)
    - MACD (доп. фильтр)
    Ожидаем колонки: open, high, low, close.
    """
    df = df.copy()
    ...
    # SMA для тренда (долгосрочный фильтр)
    df["SMA_TREND"] = close.rolling(
        window=cfg.SMA_TREND_PERIOD,
        min_periods=cfg.SMA_TREND_PERIOD
    ).mean()
    ...
    plus_dm_smooth = pd.Series(plus_dm, index=df.index).rolling(
        window=cfg.ADX_PERIOD, min_periods=cfg.ADX_PERIOD
    ).sum()
